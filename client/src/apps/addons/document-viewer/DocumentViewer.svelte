<script>
  ...
  import { fetchDocumentText } from './api.js';
  ...
  function collectMatchOffsets(source, query) {
    if (!query) return [];
  ...
  let renderedTextHtml = $derived.by(() => {
    ...
    const escapedQueryLength = query.length;
    let cursor = 0;
    let html = '';
    for (let i = 0; i < matchOffsets.length; i += 1) {
      const offset = matchOffsets[i];
      const before = textContent.slice(cursor, offset);
      const matchText = textContent.slice(offset, offset + escapedQueryLength);
      html += escapeHtml(before);
      html += `<mark class="${i === currentMatchIndex ? 'active' : ''}" data-match-index="${i}">${escapeHtml(matchText)}</mark>`;
      cursor = offset + escapedQueryLength;
    }
    html += escapeHtml(textContent.slice(cursor));
    return html;
  });
