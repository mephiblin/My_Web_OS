...
- WebDAV/rclone 원격: Cloud에 연결된 원격은 File Station 사이드바에서 로컬 폴더처럼 탐색하고, 지원 앱으로 열 수 있습니다.
- 업로드/휴지통/공유: 파일 이동과 복구, 링크 공유를 운영형으로 처리.
- 권한 경계: 앱이 필요한 파일만 다루도록 경로/권한을 제한.

### 3) 원격 컴퓨터 작업

- Terminal: 브라우저에서 실시간 터미널 세션 사용. 백엔드가 재시작되면 기존 shell은 종료되며 새 터미널 창/재연결에서 새 shell이 시작됩니다.
- Resource Monitor: CPU/메모리/시스템 상태를 확인. 반복 조회는 서버 캐시를 사용해 앱 로딩과 데스크톱 반응성을 보호합니다.
...
- WebDAV 마운트 탐색: 연결된 remote는 `cloud://` 경로로 표시되며 문서/이미지/미디어/코드/모델 파일을 기존 앱으로 엽니다.
...
- WebDAV/rclone 원격 파일은 현재 File Station에서 읽기/열기 중심입니다. 원격 rename/delete/overwrite 편집은 별도 승인 계약이 마련되기 전까지 제한됩니다.
- 대형 번들 로딩: Monaco/Three는 초기 번들에서 분리됐지만 첫 실행 로딩이 큽니다.
- 백엔드 재시작: 열려 있던 터미널 local shell은 유지되지 않습니다. 서버 재시작 후 터미널을 새로 열어야 합니다.
- 샌드박스 앱 로딩: 앱 entry/SDK가 준비되지 않으면 무한 대기 대신 명시적인 로딩/타임아웃 상태를 표시합니다.
