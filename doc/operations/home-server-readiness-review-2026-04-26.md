...
2026-04-26 HSR completion pass와 approval self-approval hardening pass 이후에는
addon-only/core-freeze mode가 조건부로 가능하다. 단, 이 뜻은 core를 버린다는 의미가 아니라 core 변경을
...
- VPN/내부망 개인 운영: `8.3 / 10`
- DSM-like 홈서버 운영면: `7.4 / 10`
- 애드온 전용 개발 전환 준비도: `90 / 100`
...
| Host path/file 보안 | 조건부 | 7.5 | 주요 경로에 realpath/symlink escape 방어가 들어갔고 sandbox overwrite approval은 parent-owned flow로 이동했다. 일부 backup path 검증은 향후 hardening 항목으로 남는다. |
| 위험 작업 승인/감사 | 조건부 통과 | 8.0 | HSR completion pass와 self-approval hardening 기준 file delete, empty trash, Docker stop/restart/remove, package install/update/import/rollback/manifest update, terminal session start, sandbox overwrite가 backend approval nonce contract를 사용한다. |
...
| 대용량/공유/미디어/클라우드 연속성 | 조건부 양호 | 8.0 | ticket/lease/share Range/A-owned cloud transfer가 분리되어 있고 overwrite-required cloud transfer는 사용자 입력 typed-confirm approval flow를 통해 완료할 수 있다. |
| Package Center/lifecycle | 조건부 통과 | 8.1 | install/update/import/rollback/manifest update/delete가 preflight + typed confirmation + consume-once approval nonce를 사용한다. Package Center UI도 typed confirmation을 사용자가 입력해야 approve endpoint를 호출한다. |
| Web Desktop/launcher | 양호 | 8.0 | registry 기반 app loading과 Spotlight 통합은 좋은 상태다. |
| Docker/Terminal | 조건부 통과 | 7.6 | Docker high-impact actions와 Terminal session start는 backend approval evidence와 사용자 입력 typed confirmation을 요구한다. raw interactive shell 권한 자체는 owner-only beta 위험으로 남는다. |
| API error/UI state | 양호 | 7.5 | `ApiError`와 structured server error가 많이 정리됐다. 일부 legacy auth/cloud/system 응답은 남아 있다. |
| Verification/CI | 양호 | 8.4 | `npm run verify`와 dependency-free `npm run verify:ui-smoke`가 현재 release gate이며 self-approval 회귀 문자열을 차단한다. Full browser click-through automation은 향후 hardening enhancement다. |
